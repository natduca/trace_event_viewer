...
# better. :)
#
# This bit of trickery is because I want the actual fact that trace_viewer
...
def try_to_exec_stub(main_name, tried_once=False):
  ...
  if objc_found:
    ...
    if not os.path.exists(trace_viewer_stub_app):
      if tried_once:
...
def is_inside_stub_bundle():
  ...
  return b.infoDictionary()["CFBundleName"] == 'trace_viewer_stub'
...
def create_stub():
  ...
  print "Creating trace_view_stub... please be patient..."
  setup(
    script_args=["py2app"],
